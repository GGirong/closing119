            ...
            <div class="row col-12 col-md-6">
              ...
              <div class="col-10  section-space--bottom--20">
                <input
                  ...
                  type="text"
                  placeholder=""
                  v-model="idData.email"
                />
              </div>
              <div v-if="findingId" class="line"></div>
              <div
                v-if="findingId"
                class="input-title col-12  section-space--bottom--20"
              >
                해당 이메일로 가입한 아이디
              </div>
...
export default {
  ...
  methods: {
    async idFind() {
      await axios
        .post("https://new-api.closing119.com/api/recoveryid/", this.idData)
        .then((res) => {
          this.findingId = true;
    ...
    async pwFind() {
      this.isLoading = true;
      await axios
        .post("https://new-api.closing119.com/api/recoverypw/", this.pwData)
        .then((res) => {
          this.isLoading = false;
          alert(
            "입력하신 이메일로 인증번호가 발송되었습니다. 메일을 확인해주세요."
          );
          this.findingPw = true;
        })
        .catch((err) => {
          if (err.response.data.code == "WRONG_USERNAME") {
            ...
            this.isLoading = false;
            alert(
              "아이디와 이메일이 일치하지 않습니다. 다시 한 번 확인해주세요."
            );
          }
        });
    },
    async changePw() {
      ...
      await axios
        .post("https://new-api.closing119.com/api/recoverycheck/", checkData)
        .then((res) => {
          alert("정상 인증되었습니다. 비밀번호 변경페이지로 안내합니다.");
          this.$router.push({
            name: "PwChange",
            params: { username: this.pwData.username, find: true },
          });
        })
        .catch((err) => {
